...
- [ ] T024 [P] [US1] Create SignUpModal component in book-source/src/components/Auth/SignUpModal.tsx
- [ ] T025 [P] [US1] Create email/password form with name, email, password fields in SignUpModal.tsx
- [ ] T026 [P] [US1] Add form validation (email format, password min 8 chars) in SignUpModal.tsx
- [ ] T027 [US1] Implement signUp.email() call in SignUpModal.tsx with onRequest, onSuccess, onError callbacks
- [ ] T028 [US1] Add loading state during sign-up (disable form, show spinner) in SignUpModal.tsx
- [ ] T029 [US1] Display "Check your email" message after successful sign-up in SignUpModal.tsx
- [ ] T030 [US1] Handle error states (email exists, weak password) with user-friendly messages in SignUpModal.tsx
- [ ] T031 [P] [US1] Configure email verification in auth-service/auth.ts (24-hour token expiration)
- [ ] T032 [P] [US1] Setup email service provider (Resend recommended) in auth-service for verification emails
- [ ] T033 [US1] Create email verification handler route in auth-service for /api/auth/verify-email
- [ ] T034 [US1] Implement redirect to dashboard after successful email verification in auth-service
- [ ] T035 [P] [US1] Add "Sign Up" button in book-source/src/components/Navbar (swizzle Navbar component)
- [ ] T036 [US1] Wire Sign Up button to open SignUpModal component
- [ ] T037 [P] [US1] Create Dashboard page component in book-source/src/pages/dashboard/index.tsx
- [ ] T038 [US1] Display user name and email on Dashboard using useSession hook
- [ ] T039 [US1] Add "Resend verification email" option for unverified users in SignUpModal.tsx
...
- [ ] T040 [P] [US2] Create SignInModal component in book-source/src/components/Auth/SignInModal.tsx
- [ ] T041 [P] [US2] Create email/password form in SignInModal.tsx
- [ ] T042 [US2] Implement signIn.email() call with email, password, rememberMe checkbox in SignInModal.tsx
- [ ] T043 [US2] Add "Remember me" checkbox (7-day session vs session cookie) in SignInModal.tsx
- [ ] T044 [US2] Handle successful sign-in redirect to dashboard in SignInModal.tsx
- [ ] T045 [US2] Display error message "Invalid email or password" without email enumeration in SignInModal.tsx
- [ ] T046 [US2] Add loading state during sign-in (disable form, spinner) in SignInModal.tsx
- [ ] T047 [P] [US2] Check for active session and auto-redirect authenticated users from sign-in page
- [ ] T048 [P] [US2] Add "Sign In" button in Navbar component
- [ ] T049 [US2] Wire Sign In button to open SignInModal component
- [ ] T050 [US2] Prevent sign-in for unverified email addresses with "Please verify your email" error in auth-service
- [ ] T051 [US2] Add link to sign-up modal from sign-in modal ("Don't have an account? Sign up")
...
- [ ] T052 [P] [US3] Register GitHub OAuth application at https://github.com/settings/developers
- [ ] T053 [P] [US3] Register Google OAuth application in Google Cloud Console
- [ ] T054 [P] [US3] Add GitHub OAuth credentials to auth-service/.env (GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET)
- [ ] T055 [P] [US3] Add Google OAuth credentials to auth-service/.env (GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET)
- [ ] T056 [P] [US3] Create SSOButtons component in book-source/src/components/Auth/SSOButtons.tsx
- [ ] T057 [P] [US3] Add "Continue with GitHub" button in SSOButtons.tsx
- [ ] T058 [P] [US3] Add "Continue with Google" button in SSOButtons.tsx
- [ ] T059 [US3] Implement signIn.social({ provider: "github" }) in GitHub button click handler
- [ ] T060 [US3] Implement signIn.social({ provider: "google" }) in Google button click handler
- [ ] T061 [US3] Handle OAuth redirect to dashboard after successful authorization in auth-service
- [ ] T062 [US3] Populate user profile (name, avatar) from OAuth provider data in auth-service
- [ ] T063 [US3] Mark OAuth users' emails as verified automatically in auth-service
- [ ] T064 [US3] Handle OAuth errors (user denies access) with "Authentication cancelled" message in SSOButtons.tsx
- [ ] T065 [P] [US3] Add SSOButtons component to SignUpModal above email form with "Or continue with email" separator
- [ ] T066 [P] [US3] Add SSOButtons component to SignInModal above email form
- [ ] T067 [US3] Implement account linking when OAuth email matches existing email account in auth-service
- [ ] T068 [US3] Prevent duplicate provider linking (same GitHub account linked twice) in auth-service
...
- [ ] T069 [P] [US4] Create RequireAuth HOC component in book-source/src/components/Auth/RequireAuth.tsx
- [ ] T070 [US4] Implement useSession check in RequireAuth with loading, error, unauthenticated states
- [ ] T071 [US4] Add redirect to sign-in modal when unauthenticated user accesses protected content
- [ ] T072 [US4] Preserve intended destination URL for redirect after authentication in RequireAuth
- [ ] T073 [P] [US4] Create AuthPromptModal component in book-source/src/components/Auth/AuthPromptModal.tsx
- [ ] T074 [US4] Display "Sign in to use the AI Assistant" message in AuthPromptModal
- [ ] T075 [US4] Add sign-in and sign-up options in AuthPromptModal
...
- [ ] T080 [US4] Handle session expiration during protected feature use with "Session expired" modal
- [ ] T081 [US4] Implement one-click re-authentication for expired sessions in AuthPromptModal
...
- [ ] T082 [P] [US5] Create UserMenu dropdown component in book-source/src/components/Auth/UserMenu.tsx
- [ ] T083 [US5] Display user avatar and name in Navbar when authenticated using useSession
- [ ] T084 [US5] Show dropdown menu with Profile, Settings, Sign Out options on avatar click
- [ ] T085 [P] [US5] Create Profile page component in book-source/src/pages/profile/index.tsx
- [ ] T086 [US5] Add form to update display name in Profile page
- [ ] T087 [US5] Implement name update with session refetch() to sync UI in Profile page
- [ ] T088 [P] [US5] Create AccountSettings page component in book-source/src/pages/settings/account.tsx
- [ ] T089 [US5] Display all linked authentication methods using useListAccounts hook in AccountSettings
- [ ] T090 [US5] Add "Link GitHub" button in AccountSettings for users without GitHub
- [ ] T091 [US5] Add "Link Google" button in AccountSettings for users without Google
- [ ] T092 [US5] Implement linkSocial({ provider: "github" }) for linking additional OAuth accounts
- [ ] T093 [US5] Implement linkSocial({ provider: "google" }) for linking additional OAuth accounts
- [ ] T094 [US5] Add "Unlink" button for secondary authentication methods in AccountSettings
- [ ] T095 [US5] Implement unlinkAccount({ accountId }) with validation (must keep 1 method) in AccountSettings
- [ ] T096 [US5] Add "Sign out" functionality in UserMenu using authClient.signOut()
- [ ] T097 [US5] Add "Sign out from all devices" option in AccountSettings with session revocation
- [ ] T098 [US5] Clear session cookie and redirect to homepage on sign out
...
- [ ] T099 [P] [US6] Create PasswordResetRequest component in book-source/src/components/Auth/PasswordResetRequest.tsx
- [ ] T100 [US6] Add email input form in PasswordResetRequest component
- [ ] T101 [US6] Implement forgetPassword({ email }) call in PasswordResetRequest
- [ ] T102 [US6] Display "If that email exists, you'll receive a reset link" message (no enumeration)
- [ ] T103 [P] [US6] Configure password reset in auth-service with 1-hour token expiration
- [ ] T104 [P] [US6] Setup password reset email template in auth-service email provider
- [ ] T105 [P] [US6] Create PasswordResetConfirm component in book-source/src/pages/reset-password/[token].tsx
- [ ] T106 [US6] Add new password input form in PasswordResetConfirm
- [ ] T107 [US6] Validate reset token in PasswordResetConfirm component
- [ ] T108 [US6] Implement resetPassword({ newPassword, token }) in PasswordResetConfirm
- [ ] T109 [US6] Handle expired reset link (>1 hour) with "Link expired" message in PasswordResetConfirm
- [ ] T110 [US6] Handle already-used reset link with "Link already used" message in PasswordResetConfirm
- [ ] T111 [US6] Revoke all other sessions when password is successfully reset in auth-service
- [ ] T112 [US6] Redirect to sign-in page after successful password reset
- [ ] T113 [US6] Add "Forgot password?" link in SignInModal to open PasswordResetRequest
...
- [ ] T114 [P] [US7] Configure session cookie settings in auth-service (httpOnly, secure, sameSite: lax)
- [ ] T115 [P] [US7] Implement "Remember me" logic in auth-service (7-day vs session cookie expiration)
- [ ] T116 [US7] Test session persistence on page refresh using useSession hook in Dashboard
- [ ] T117 [US7] Prevent flash of unauthenticated content (FOUC) using isPending state in RequireAuth
- [ ] T118 [US7] Add skeleton loader during session restoration in Dashboard component
- [ ] T119 [US7] Configure session expiration after 7 days of inactivity in auth-service
- [ ] T120 [US7] Display "Session expired, please sign in again" modal when session expires
- [ ] T121 [US7] Clear session cookie on explicit sign-out in auth-service
- [ ] T122 [US7] Redirect to homepage after explicit sign-out in frontend
...
- [ ] T123 [P] Add Tailwind CSS styling to all auth components (SignUpModal, SignInModal, SSOButtons, etc.)
- [ ] T124 [P] Implement toast notifications for success/error feedback across all auth flows
- [ ] T125 [P] Add loading skeletons for all auth-related loading states
- [ ] T126 [P] Ensure all modals are responsive (mobile, tablet, desktop)
- [ ] T127 [P] Add proper focus management and keyboard navigation in auth modals
- [ ] T128 [P] Implement proper error logging for auth failures in auth-service
- [ ] T129 Test all user stories end-to-end in development environment
- [ ] T130 [P] Update README.md with auth service setup instructions
- [ ] T131 [P] Create .env.example files for both book-source and auth-service
- [ ] T132 [P] Add TypeScript types for all auth-related components
...
- [ ] T136 Create development script to run both Docusaurus (3000) and auth-service (8002) concurrently
